...
# The resulting object "kaplan" is a "survfit" object that stores the Kaplan-Meier estimates of the survival probabilities, 
# as well as the number of observations and the number of events at each time point. 
# The object can be used for further analysis or plotting.
...
# this command (below) performs a Cox proportional hazards regression analysis to estimate the hazard 
# rate of an event over time, while controlling for the effects of age and sex in the "infants" data frame. 
# This is a common technique used in survival analysis to model the effects of predictor variables on 
# the risk of an event, and to identify factors that may be associated with increased or decreased risk.
